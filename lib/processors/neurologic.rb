module Processor
  class Neurologic
    ...
    def check_for_delirium patient
      ...
      has_data[0], is_met[0], relevant_observations[0] = GuidelineManager::process_guideline_step(patient, ["delirium_screening"], pg, 0, Helper.latest_code_exists_proc, Helper.observation_yes_check)
      has_data[1], is_met[1], relevant_observations[1] = GuidelineManager::process_guideline_step(patient, ["LP74647-6"], pg, 0, Helper.any_code_exists_proc, positive_delirium_check) unless is_met[0]

      observations = Hash.new
      observations["Positive Neurologic Screening"] = relevant_observations[0]
      observations["CAM ICU"] = relevant_observations[1]
    ...
    def check_for_alcohol_withdrawal patient
      ...
      has_data[0], is_met[0], relevant_observations[0] = GuidelineManager::process_guideline_step(patient, ["ciwa_score", "445504001"], pg, 0, Helper.any_code_exists_proc, alcohol_withdrawal_check)

      observations = Hash.new
      observations["CIWA Score"] = relevant_observations[0]
    ...
    def check_for_altered_mental_status patient
      ...
      has_data[0], is_met[0], relevant_observations[0] = GuidelineManager::process_guideline_step(patient, ["glasgow_coma_decrease"], pg, 0, Helper.latest_code_exists_proc, Helper.observation_yes_check)
      has_data[1], is_met[1], relevant_observations[1] = GuidelineManager::process_guideline_step(patient, ["glasgow_coma_scale", "glasgow_score", "386557006"], pg, 0, Helper.any_code_exists_proc, coma_baseline_check) unless is_met[0]

      observations = Hash.new
      observations["Coma Scale Decreased by >= 2 pts?"] = relevant_observations[0]
      observations["Glascow Score"] = relevant_observations[1]
      GuidelineManager::update_step(Processor::Helper.find_guideline_step(pg, 0), (is_met[0] or is_met[1]), !(has_data[0] or has_data[1]), observations)

      has_data[2], is_met[2], relevant_observations[2] = GuidelineManager::process_guideline_step(patient, ["glasgow_coma_scale", "glasgow_score", "386557006"], pg, 1, Helper.any_code_exists_proc, coma_threshold_check) unless (is_met[0] or is_met[1])

      observations = Hash.new
      observations["Glascow Score"] = relevant_observations[2]
